...
module Walrus
  ...
  class Parser
    ...
    @@grammar ||= Grammar.subclass('WalrusGrammar') do
      ...
      # only after defining the grammar is it safe to extend the classes
      # dynamically created during the grammar definition
      # TODO: modify the code which creates classes on the fly so that the
      # requires are no longer order-sensitive
